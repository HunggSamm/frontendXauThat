import {
  ...
  const Curriculum = () => {
    ...
    const [isAddingNewSection, setIsAddingNewSection] = useState(false); // Track if adding new section
    const [newSectionTitle, setNewSectionTitle] = useState(""); // Track new section title
    ...
    const handleAddNewSection = () => {
      if (newSectionTitle.trim() !== "") {
        const newSection = {
          id: sections.length + 1, // Incremental ID
          title: newSectionTitle,
          lessons: [],
        };
        setSections([...sections, newSection]); // Add new section to state
        setNewSectionTitle(""); // Clear the input
        setIsAddingNewSection(false); // Exit adding mode
      }
    };
          ...
          {isAddingNewSection ? (
            <VStack mt={4} spacing={3}>
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    handleAddNewSection(); // Add new section on Enter key press
                  }
                }}
                autoFocus
              />
